...
final class RabbitMQAdapter implements MessageBusInterface
{
    ...
    {
        error_log("RabbitMQAdapter::publishEvent called with data: " . json_encode($eventData));
        $result = $this->publish('events', 'published', $eventData);
        error_log("RabbitMQAdapter::publishEvent result: " . ($result ? 'SUCCESS' : 'FAILED'));
        return $result;
    }
    ...
    public function publishNotification(array $notificationData, ?string $userId = null): bool
    {
        ...
        error_log("RabbitMQAdapter::publishNotification called with routing key '{$routingKey}' and data: " . json_encode($notificationData));
        $result = $this->publish('notifications', $routingKey, $notificationData);
        error_log("RabbitMQAdapter::publishNotification result: " . ($result ? 'SUCCESS' : 'FAILED'));
        return $result;
    }
